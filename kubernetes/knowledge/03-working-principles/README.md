...
| 组件/机制 | 复杂度 | 核心算法 | 性能特性 |
|-----------|--------|----------|----------|
| [API Server 请求处理](./api-server.md) | 中级 | 流水线处理 | 高并发、低延迟 |
| [认证授权机制](./authentication.md) | 中级 | 权限检查算法 | 安全性优先 |
| [准入控制器](./admission-control.md) | 高级 | 策略评估引擎 | 可扩展性强 |
| [资源版本控制](./resource-versioning.md) | 中级 | 乐观锁算法 | 并发控制 |

### 调度层工作原理
| 组件/机制 | 复杂度 | 核心算法 | 性能特性 |
|-----------|--------|----------|----------|
| [调度器算法](./scheduler.md) | 高级 | 多阶段调度算法 | 可扩展、可配置 |
| [节点过滤算法](./node-filtering.md) | 中级 | 约束满足算法 | 高效筛选 |
| [节点评分算法](./node-scoring.md) | 中级 | 多目标优化 | 负载均衡 |
| [抢占调度机制](./preemption.md) | 高级 | 优先级调度 | 资源优化 |

### 控制层工作原理
| 组件/机制 | 复杂度 | 核心算法 | 性能特性 |
|-----------|--------|----------|----------|
| [控制器协调循环](./controller-loop.md) | 高级 | 事件驱动循环 | 自愈能力强 |
| [垃圾回收机制](./garbage-collection.md) | 中级 | 图遍历算法 | 资源清理 |
| [资源依赖管理](./resource-dependency.md) | 中级 | 层次关系维护 | 级联操作 |
| [状态同步机制](./state-sync.md) | 高级 | 分布式一致性 | 数据一致性 |

### 执行层工作原理
| 组件/机制 | 复杂度 | 核心算法 | 性能特性 |
|-----------|--------|----------|----------|
| [kubelet Pod 管理](./kubelet-pod-management.md) | 高级 | 生命周期管理 | 节点级编排 |
| [容器运行时接口](./container-runtime-interface.md) | 中级 | CRI 协议 | 运行时抽象 |
| [网络代理机制](./network-proxy.md) | 中级 | 流量代理算法 | 服务发现 |
| [卷管理机制](./volume-management.md) | 中级 | 存储抽象 | 数据持久化 |

### 存储层工作原理
| 组件/机制 | 复杂度 | 核心算法 | 性能特性 |
|-----------|--------|----------|----------|
| [etcd 一致性算法](./etcd-consistency.md) | 高级 | Raft 共识算法 | 强一致性 |
| [Watch 机制原理](./watch-mechanism.md) | 中级 | 事件流处理 | 实时通知 |
| [数据序列化](./data-serialization.md) | 基础 | Protobuf/JSON | 高效传输 |
| [缓存机制](./cache-mechanism.md) | 中级 | 多级缓存 | 性能优化 |
...
### 初学者路径
1. **基础概念理解**：从 [调度器算法](./scheduler.md) 开始，理解 Kubernetes 的调度决策过程
2. **状态管理机制**：学习 [控制器协调循环](./controller-loop.md)，掌握状态驱动的核心思想
3. **执行层原理**：深入 [kubelet Pod 管理](./kubelet-pod-management.md)，了解 Pod 的完整生命周期

### 进阶路径
1. **API 层深入**：研究 [API Server 请求处理](./api-server.md) 和 [准入控制器](./admission-control.md)
2. **存储层原理**：掌握 [etcd 一致性算法](./etcd-consistency.md) 和 [Watch 机制原理](./watch-mechanism.md)
3. **网络存储机制**：掌握 [网络代理机制](./network-proxy.md) 和 [卷管理机制](./volume-management.md)
4. **高级调度功能**：学习 [节点过滤算法](./node-filtering.md) 和 [抢占调度机制](./preemption.md)

### 专家路径
1. **存储优化专精**：深入 [数据序列化](./data-serialization.md) 和 [缓存机制](./cache-mechanism.md)
2. **源码级理解**：深入各文档的源码实现部分
3. **性能优化**：重点关注各组件的性能优化技巧
4. **扩展开发**：学习如何基于这些原理开发自定义控制器和插件
